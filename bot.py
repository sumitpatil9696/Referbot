...
def open_panel(msg):
    if msg.from_user.id != ADMIN_ID:
        return
    bot.send_message(msg.chat.id, "⚙️ Advanced Panel", reply_markup=advanced_admin_menu())
...
def pending_list(msg):
    ...
    for uid, amt in pending_withdraw.items():
        text += f"{uid} → ₹{amt}\n"
    bot.send_message(msg.chat.id, text if text else "No pending")

@bot.message_handler(func=lambda m: m.text == "🔙 Back Admin")
def back(msg):
    if msg.from_user.id != ADMIN_ID:
        return
    bot.send_message(msg.chat.id, "Back", reply_markup=admin_menu())
